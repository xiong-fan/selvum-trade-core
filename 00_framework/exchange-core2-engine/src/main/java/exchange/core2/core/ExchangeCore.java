...
public final class ExchangeCore {
    ...
    private final Map<Integer, RiskEngine> riskEngines;
    private final Map<Integer, MatchingEngineRouter> matchingEngines;
    ...
    public ExchangeCore(final ObjLongConsumer<OrderCommand> resultsConsumer,
        ...
        this.matchingEngines = Collections.unmodifiableMap(
                matchingEngineFutures.entrySet().stream()
                        .collect(Collectors.toMap(
                                Map.Entry::getKey,
                                entry -> entry.getValue().join())));

        final EventHandler<OrderCommand>[] matchingEngineHandlers = this.matchingEngines.values().stream()
                .map(mer -> (EventHandler<OrderCommand>) (cmd, seq, eob) -> mer.processOrder(seq, cmd))
                .toArray(ExchangeCore::newEventHandlersArray);

        this.riskEngines = Collections.unmodifiableMap(
                riskEngineFutures.entrySet().stream()
                        .collect(Collectors.toMap(
                                Map.Entry::getKey,
                                entry -> entry.getValue().join())));
    ...
    /**
     * Returns risk engines (unmodifiable map, shardId → RiskEngine).
     * Each RiskEngine provides access to UserProfileService and other per-shard state.
     *
     * @return unmodifiable map of risk engines
     */
    public Map<Integer, RiskEngine> getRiskEngines() {
        return riskEngines;
    }

    public Map<Integer, MatchingEngineRouter> getMatchingEngines() {
