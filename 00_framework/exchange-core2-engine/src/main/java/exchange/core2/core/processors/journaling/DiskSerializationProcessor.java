...
//        log.info("Read total: {} bytes ", totalBytesRead);
        ...
        while (true) {
            ...
            try (final FileInputStream fis = new FileInputStream(path.toFile());
                ...
                // 同步 filesCounter，避免新写入覆盖已被重放的 journal 文件
                filesCounter = partitionCounter - 1;
                log.debug("return lastSeq={}, file not found: {}, filesCounter={}", lastSeq, ex.getMessage(), filesCounter);
                return lastSeq.value;

            } catch (IOException ex) {
                partitionCounter++;
                filesCounter = partitionCounter - 1;
                log.debug("File end reached through exception, filesCounter={}", filesCounter);
            }
        }
    }
...
//        }
        ...
        if (channel == null && journalWriteBuffer.position() == 0 && !forceStartNextFile) {
            log.debug("Skipping empty journal flush because no journal file has been started yet");
            return;
        }
