...
public interface ExchangeOrderRepository extends JpaRepository<ExchangeOrder, String>, JpaSpecificationExecutor<ExchangeOrder>, QueryDslPredicateExecutor<ExchangeOrder> {
    ...
    @Modifying
    @Query("update ExchangeOrder exchange set exchange.tradedAmount = exchange.tradedAmount + :amount, exchange.turnover = exchange.turnover + :turnover where exchange.orderId = :orderId")
    int increaseTradeAmountAndTurnover(@Param("orderId") String orderId, @Param("amount") BigDecimal amount, @Param("turnover") BigDecimal turnover);
