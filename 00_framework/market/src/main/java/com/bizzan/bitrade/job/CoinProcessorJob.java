...
public class CoinProcessorJob {
    ...
    public void synchronizeExchangeCenter(){
        ...
        ResponseEntity<HashMap> resultStr;
        try {
            resultStr = restTemplate.getForEntity(url, HashMap.class);
        } catch (Exception e) {
            log.warn("========CoinProcessorJob========SERVICE-EXCHANGE-TRADE not available, skip sync. This is expected when using matching-core-service.");
            return;
        }
