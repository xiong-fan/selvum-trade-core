...
public class ExchangeTradeConsumer {
	...
	public void handleMatchingTradeEvent(List<ConsumerRecord<String, String>> records) {
		List<ExchangeTrade> trades = new java.util.ArrayList<ExchangeTrade>();
		for (ConsumerRecord<String, String> record : records) {
			MatchingTradeEvent event = JSON.parseObject(record.value(), MatchingTradeEvent.class);
			...
			trade.setBuyTurnover(event.getTurnover());
			trade.setSellTurnover(event.getTurnover());
			trade.setBuyOrderId(event.getTakerOrderId());
			trade.setSellOrderId(event.getMakerOrderId());
			trade.setDirection(ExchangeOrderDirection.BUY);
			trade.setTime(event.getTimestamp());
			trades.add(trade);
		}
		if (trades.isEmpty()) {
			return;
	...
	@KafkaListener(topics = MatchingTopics.MATCHING_ORDERBOOK_EVENT, containerFactory = "kafkaListenerContainerFactory")
	public void handleMatchingOrderBookEvent(List<ConsumerRecord<String, String>> records) {
		for (ConsumerRecord<String, String> record : records) {
			logger.info("matching orderbook event received, key={}, value={}", record.key(), record.value());
		}
	}

	@KafkaListener(topics = MatchingTopics.MATCHING_ORDER_EVENT, containerFactory = "kafkaListenerContainerFactory")
	public void handleMatchingOrderEvent(List<ConsumerRecord<String, String>> records) {
		for (ConsumerRecord<String, String> record : records) {
			logger.info("matching order event received, key={}, value={}", record.key(), record.value());
		}
	}
