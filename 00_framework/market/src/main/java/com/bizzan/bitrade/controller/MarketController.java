...
public class MarketController {
    ...
    public Object findTradePlateFull(String symbol){
        //远程RPC服务URL,后缀为币种单位
        String serviceName = "SERVICE-EXCHANGE-TRADE";
    ...
    private Object getExchangeTradeServiceBody(String url) {
        try {
            ResponseEntity<HashMap> result = restTemplate.getForEntity(url, HashMap.class);
            return result.getBody();
        }
        catch (RestClientException e) {
            log.warn("SERVICE-EXCHANGE-TRADE unavailable: {}", url, e);
            MessageResult result = MessageResult.error("SERVICE-EXCHANGE-TRADE unavailable, please start exchange matching service and check Eureka registration");
            result.setCode(500);
            return result;
        }
    }
