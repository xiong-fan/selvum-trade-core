...
public class OrderController {
    ...
    @Value("${matching.core.enabled:false}")
    private boolean matchingCoreEnabled;
    @Autowired
    ...
    @Autowired
    private CoreSymbolMappingRepository coreSymbolMappingRepository;
...
//        }
    ...
    public boolean isExchangeOrderExist(ExchangeOrder order){
        if (matchingCoreEnabled) {
            return coreOrderMappingService.findByOrderId(order.getOrderId()) != null;
        }
        try {
    ...
    private void sendPlaceOrder(ExchangeOrder order) {
        if (!matchingCoreEnabled) {
            kafkaTemplate.send("exchange-order", JSON.toJSONString(order));
            return;
        }
        // 确保交易对已在撮合引擎中注册（重复注册幂等）
        CoreSymbolMapping symbolMapping = coreSymbolMappingRepository.findBySymbol(order.getSymbol());
        if (symbolMapping != null) {
            MatchingCommand addSymbol = new MatchingCommand();
            addSymbol.setCommandId("ADD_SYMBOL-" + order.getSymbol());
            addSymbol.setCommandType(MatchingCommandType.ADD_SYMBOL);
            addSymbol.setSymbol(order.getSymbol());
            addSymbol.setCoreSymbolId(symbolMapping.getCoreSymbolId());
            addSymbol.setTimestamp(System.currentTimeMillis());
            kafkaTemplate.send(MatchingTopics.MATCHING_COMMAND, order.getSymbol(), JSON.toJSONString(addSymbol));
        }

        // 确保用户已在撮合引擎中注册（重复注册幂等）
        MatchingCommand addUser = new MatchingCommand();
        addUser.setCommandId("ADD_USER-" + order.getMemberId());
        addUser.setCommandType(MatchingCommandType.ADD_USER);
        addUser.setMemberId(order.getMemberId());
        addUser.setTimestamp(System.currentTimeMillis());
        kafkaTemplate.send(MatchingTopics.MATCHING_COMMAND, order.getSymbol(), JSON.toJSONString(addUser));

        CoreOrderMapping mapping = coreOrderMappingService.createIfAbsent(order);
        MatchingCommand command = buildOrderCommand(MatchingCommandType.PLACE_ORDER, order, mapping);
        kafkaTemplate.send(MatchingTopics.MATCHING_COMMAND, order.getSymbol(), JSON.toJSONString(command));
    }

    private void sendCancelOrder(ExchangeOrder order) {
        if (!matchingCoreEnabled) {
            kafkaTemplate.send("exchange-order-cancel", JSON.toJSONString(order));
            return;
        }
        CoreOrderMapping mapping = coreOrderMappingService.findByOrderId(order.getOrderId());
        if (mapping == null) {
            throw new IllegalStateException("core order mapping not found: " + order.getOrderId());
        }
        MatchingCommand command = buildOrderCommand(MatchingCommandType.CANCEL_ORDER, order, mapping);
        kafkaTemplate.send(MatchingTopics.MATCHING_COMMAND, order.getSymbol(), JSON.toJSONString(command));
    }

    private MatchingCommand buildOrderCommand(MatchingCommandType type, ExchangeOrder order, CoreOrderMapping mapping) {
        MatchingCommand command = new MatchingCommand();
        command.setCommandId(type.name() + "-" + order.getOrderId() + "-" + System.currentTimeMillis());
        command.setCommandType(type);
        command.setBusinessKey(order.getOrderId());
        command.setMemberId(order.getMemberId());
        command.setSymbol(order.getSymbol());
        command.setOrderId(order.getOrderId());
        command.setCoreOrderId(mapping.getCoreOrderId());
        command.setCoreSymbolId(mapping.getCoreSymbolId());
        command.setDirection(order.getDirection());
        command.setOrderType(order.getType());
        command.setPrice(order.getPrice());
        command.setAmount(order.getAmount());
        command.setTimestamp(System.currentTimeMillis());
        if (type == MatchingCommandType.PLACE_ORDER) {
            command.setCommandId("PLACE_ORDER-" + order.getOrderId());
        } else if (type == MatchingCommandType.CANCEL_ORDER) {
            command.setCommandId("CANCEL_ORDER-" + order.getOrderId());
        }
        return command;
    }

    private boolean isCancelableStatus(ExchangeOrderStatus status) {
        if (!matchingCoreEnabled) {
            return status == ExchangeOrderStatus.TRADING;
        }
        return status == ExchangeOrderStatus.SUBMITTED
                || status == ExchangeOrderStatus.ACCEPTED
                || status == ExchangeOrderStatus.PARTIALLY_TRADED
